...
if [ $folder_name = 'generic' ]; then {
	if [ $jmx_file = 'eas' ]; then {
		echo "${jmeter} -n -Juser.classpath=${workspace}/src/build/jar/zjmeter.jar:${workspace}/src/build/jar/zm-sync-common.jar -t ${workspace}/tests/${folder_name}/${jmx_file}/${jmx_file}.jmx -q ${workspace}/config/env.prop -q ${workspace}/tests/${folder_name}/${jmx_file}/load.prop -q ${workspace}/tests/${folder_name}/${jmx_file}/profiles/basic.prop -l ${report_dir}/${test_name}.xml -Djmeter.save.saveservice.thread_counts=true -Djmeter.save.saveservice.output_format=xml -q ${workspace}/config/extras.txt"
		${jmeter} -n -Juser.classpath="${workspace}/src/build/jar/zjmeter.jar:${workspace}/src/build/jar/zm-sync-common.jar" -t ${workspace}/tests/${folder_name}/${jmx_file}/${jmx_file}.jmx -q ${workspace}/config/env.prop -q ${workspace}/tests/${folder_name}/${jmx_file}/load.prop -q ${workspace}/tests/${folder_name}/${jmx_file}/profiles/basic.prop -l ${report_dir}/${test_name}.xml -Djmeter.save.saveservice.thread_counts=true -Djmeter.save.saveservice.output_format=xml -q ${workspace}/config/extras.txt
	}
	else {
